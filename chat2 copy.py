    ...
    while True:
        ...
        try:
            ...
            if mode_stream:
                ...
                for chunk in output:
                    print(f"{log2.NEG_BLUE}Done crunching!{log2.RESET}") if (not output_thinking and not output_answer) else None
                    if chunk.message.thinking:
                        print(f"{log2.BLUE}{chunk.message.thinking}{log2.RESET}", end='', flush=True)
                        output_thinking += chunk.message.thinking
                    elif chunk.message.content:
                        print(f"{log2.NEG_GREEN}Thinking complete!{log2.RESET}") if (output_thinking and not output_answer) else None
                        print(chunk.message.content, end='', flush=True)
                        output_answer += chunk.message.content
            else:
